...
CCR_CNTRACT_TYP_REF
 -------------------
 SELECT CAP.*
FROM ENT_ACCOUNT_PERIODIC_FACT EAPF
JOIN CCR_ACCOUNT_PERIODIC CAP ON EAPF.ACCOUNT_ID = CAP.ACCOUNT_ID
WHERE EAPF.SRCE_INST = 1
  AND EAPF.SRCE_SYS = 1359
  AND (EAPF.SRCE_PROD_CDE_FK LIKE '3%' OR EAPF.SRCE_PROD_CDE_FK LIKE '4%')
  AND (
    EAPF.CR_PORTF_FK NOT IN ('PDH', 'BTL') OR
    EAPF.PROD_SUMM_DESCR NOT IN ('PROPERTY FINANCE', 'HOME LOAN')
  )
  AND CAP.CCR_CNTRACT_TYP != 'CN';
